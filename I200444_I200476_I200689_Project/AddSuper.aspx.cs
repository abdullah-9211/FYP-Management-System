...
{
    public partial class AddSuper : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {

        }
        protected void Register_Super(object sender, EventArgs e)
        {
            ...
            string Query = "INSERT INTO [USER] VALUES('" + user + "', '" + pass + "', '" + fname + "', '" + lname + "', 'Faculty', 'Supervisor');";
            SqlCommand cmd = new SqlCommand(Query, con);
            con.Open();
            int check = cmd.ExecuteNonQuery();
            if (check != 0)
            {
                Query = "INSERT INTO [AUDIT TRAIL] VALUES('" + curr_user + "', CONVERT(DATE,'" + curr_date + "'), CONVERT(TIME, '" + time + "'), 'Added " + fname + " " + lname + "s Supervisor Account');";
